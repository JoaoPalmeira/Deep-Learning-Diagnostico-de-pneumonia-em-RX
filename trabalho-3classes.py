...
def read_dataset(path):
    ...
    paths,dirs,filesBacteria = next(os.walk(path+"/train/BACTERIA"))
    paths,dirs,filesVirus = next(os.walk(path+"/train/VIRUS"))
    #Lê os nomes dos ficheiros e concatena num array 
    files = np.concatenate((filesNormal, filesBacteria, filesVirus), axis=0)
    numFilesTreino = len(files)
    
    paths,dirs,filesNormal = next(os.walk(path+"/test/NORMAL"))
    paths,dirs,filesBacteria = next(os.walk(path+"/test/BACTERIA"))
    paths,dirs,filesVirus = next(os.walk(path+"/test/VIRUS"))
    #Lê os nomes dos ficheiros e concatena num array 
    files = np.concatenate((filesNormal, filesBacteria, filesVirus), axis=0)
    numFilesTeste = len(files)
    
    paths,dirs,filesNormal = next(os.walk(path+"/validation/NORMAL"))
    paths,dirs,filesBacteria = next(os.walk(path+"/validation/BACTERIA"))
    paths,dirs,filesVirus = next(os.walk(path+"/validation/VIRUS"))
    #Lê os nomes dos ficheiros e concatena num array 
    files = np.concatenate((filesNormal, filesBacteria, filesVirus), axis=0)
